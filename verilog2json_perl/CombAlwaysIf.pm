...
sub always_comb_if {
    ...
    if ($line1 =~ /\(([\w=!<>\?\|:&\+\-\*\/%^!~\(\)\']+)\)/) {
        $conditional_expr = $1;
        @conditional_expr1 = CheckExpr::create_postfix($conditional_expr);
    } else {
        die "Error from always_comb_if no proper expression";
    }
    $line1 =~ s/\s*\(\s*$conditional_expr\s*\)\s*//;
    ...
    $json_var->{"शर्तिः"} = join (" ",@conditional_expr1);
    if ($line1 =~ /begin/) {
        $expect_end = 1;
        $line1 =~ s/begin//;
    }
    while (1) {
        ...
        if ($line1 =~ /\s*end$|\s*end\s/ ) {
            ...
            while ($line1 !~ /;/) {
                $line_no = $line_no+1;
                if ($line_no > $VerilogParser::max_line) {
                    die "Max line reached at CombAlwaysIf statement";
                }
                $line1 .= $VerilogParser::verilog_file[$line_no];
                chomp($line1);
                $line1 =~ s/\s*//g;        
    ...
    if ($line1 =~ /^\s*else/) {
        ...
        if ($line1 =~ /if/) {
            ...
            $line1 =~ s/else//;
            $line_no = &CombAlwaysElseOnly::always_comb_elseonly($line1,$line_no);
            pop @CombAlways::always_comb_block;
            pop @CombAlways::always_comb_block;
            pop @CombAlways::always_comb_block;
            pop @CombAlways::always_comb_block;
