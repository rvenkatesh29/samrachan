...
sub check_always_comb {
    ...
    my $end_occurred = 0;
    @always_comb_block = ();
    $CombAlwaysIf::always_if_count = 0;
    push @always_comb_block, $always_count;
    my $json_statement = ${always_count}."_सदाङ्गम्";
    ...
    if ($line1 =~ /always_comb/) {
        $line1 =~ s/always_comb//;
        $json_var->{"कालाधीनम्"} = "न";
        $json_var->{"संवेदनसूची"} ="*"
    } elsif ($line1 =~ /always\s*@/) {
        $line1 =~ s/always\s*@//;
        if ($line1 =~ /\*/) {
            $line1 =~ s/\*//;
            $json_var->{"कालाधीनम्"} = "न";
            ...
            die "From combAlways ; To be updated later";
        }
    }
    ...
    my $statement_type;
    my $statement_count;
    $json_var = $JsonOutput::module_json{$VerilogParser::module_name};
    ...
    while (1) {
        ...
        if ($line1 =~ /^\s*if/) { #Check for IF loop
           ...
           $line_no = CombAlwaysIf::always_comb_if ($line1, $line_no);
           $line1 = "";
           if ($expect_end == 0) {               
                $end_occurred = 1;           
            }
